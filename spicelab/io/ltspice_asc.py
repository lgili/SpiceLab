...
# Assuming these are available from the project's core modules
...
from spicelab.library.opamps import OpAmpSubckt # Re-import OpAmpSubckt
from spicelab.core.net import GND, Net, Port
...
# Function to convert engineering notation to float.
# This is a simplified version, based on common LTspice suffixes.
def try_convert_value(value_str: str | float) -> float:
    if isinstance(value_str, (int, float)):
        return float(value_str)

    s = str(value_str).strip().lower()
    multipliers = {
        't': 1e12, 'g': 1e9, 'meg': 1e6, 'm': 1e-3, 'u': 1e-6, 'n': 1e-9, 'p': 1e-12,
        'f': 1e-15, 'a': 1e-18, 'k': 1e3, 'mil': 25.4e-6
    }
    ...
    for suffix, multiplier in multipliers.items():
        if s.endswith(suffix):
            try:
                base_val = float(s[:-len(suffix)])
                return base_val * multiplier
            except ValueError:
                pass
    
    # If all fails, raise an error
    raise ValueError(f"Could not convert '{value_str}' to a numeric value.")
...
def _build_cccs(comp: AscComponent) -> CCCS:
    ...
    gain = comp.attributes.get("Value", "1")
    return CCCS(ref, ctrl_vsrc=ctrl, gain=try_convert_value(gain)) # Corrected to use try_convert_value
...
def _build_ccvs(comp: AscComponent) -> CCVS:
    ...
    return CCVS(ref, ctrl_vsrc=ctrl, r=try_convert_value(value)) # Corrected to use try_convert_value
...
def _build_res(comp: AscComponent) -> Resistor:
    ...
    return Resistor(ref, try_convert_value(value))
...
def _build_cap(comp: AscComponent) -> Capacitor:
    ...
    return Capacitor(ref, try_convert_value(value)) # Corrected to use try_convert_value
...
def _from_spice_cap(tokens: list[str], comp: AscComponent) -> Capacitor:
    ...
    return Capacitor(ref, try_convert_value(value))
...
def _from_spice_ind(tokens: list[str], comp: AscComponent) -> Inductor:
    ...
    return Inductor(ref, try_convert_value(value))
...
def _from_spice_i(tokens: list[str], comp: AscComponent) -> Idc:
    ref = tokens[0][1:] if tokens else comp.ref or "?"
    value = tokens[3] if len(tokens) > 3 else comp.attributes.get("Value", "0")
    return Idc(ref, try_convert_value(value))
...
def _from_spice_vccs(tokens: list[str], comp: AscComponent) -> VCCS:
    ...
    return VCCS(ref, try_convert_value(gm))
...
def _from_spice_cccs(tokens: list[str], comp: AscComponent) -> CCCS:
    ...
    return CCCS(ref, ctrl_vsrc=ctrl, gain=try_convert_value(gain))
...
def _from_spice_ccvs(tokens: list[str], comp: AscComponent) -> CCVS:
    ...
    return CCVS(ref, ctrl_vsrc=ctrl, r=try_convert_value(value))


def _orient_vertical_if_ground(pin_nets: list[str]) -> str:
    if any(name.lower() in {"0", "gnd", "GND"} for name in pin_nets):
        return "R90"
    return "R0"


# Fixed rotation dictionaries
_FORWARD_ROTATION_MAP = {"R0": "R0", "R90": "R90", "R180": "R180", "R270": "R270"}
_REVERSE_ROTATION_MAP = {"R0": "R0", "R90": "R270", "R180": "R180", "R270": "R90"}
...
SYMBOL_LIBRARY: dict[str, SymbolSpec] = {
    ...
    # Use OpAmpSubckt for opamp symbols, to match the PT1000_Analysis.py script that works
    "opamp": SymbolSpec(
        symbol="opamp2",
        build_component=lambda comp: OpAmpSubckt( # Reverted to OpAmpSubckt
            ref=comp.attr("InstName", "X1") or "X1", # OpAmpSubckt typically start with X
            subckt_name=comp.attr("Value", "LM741") or "LM741", # Use Value attribute for subckt_name
        ),
        extract_attributes=lambda comp: {
            "InstName": getattr(comp, "ref", "X1"),
            "Value": getattr(comp, "value", "LM741"),
        },
        pin_offsets=(
            AscPoint(0, 0),  # non
            AscPoint(0, _PIN_SPACING),  # inv
            AscPoint(_PIN_SPACING, _PIN_SPACING // 2),  # out
            AscPoint(0, -_PIN_SPACING),  # vp
            AscPoint(0, 2 * _PIN_SPACING),  # vn
        ),
        allowed_orientations=("R0",),
    ),
    # Alias symbol names used by LTspice for the generic op-amp to our spec
    "opamp2": None,  # placeholder, will be replaced below
    r"OpAmps\UniversalOpAmp": None,  # Windows path-like symbol name in some ASCs
}
...
SYMBOL_LIBRARY[r"OpAmps\UniversalOpAmp"] = _op_spec
...
def _resolve_symbol_spec(sym: str) -> SymbolSpec | None:
    ...
    norm = sym.replace("\\\\", "/") # Corrected backslash escaping
    base = norm.split("/")[-1]
...
COMPONENT_TO_SYMBOL: dict[type[Component], SymbolSpec] = {
    ...
    # OpAmpIdeal is not directly mapped as it's represented by OpAmpSubckt (LM741)
}
...
class _UnionFind:
    ...
    def find(self, point: AscPoint) -> tuple[int, int]:
        ...
        self._parent.setdefault(coord, coord)
        parent_coord = self._parent[coord]
        if parent_coord == coord:
            return coord
        # Path compression
        self._parent[coord] = self.find(AscPoint(*parent_coord))
...
def _parse_parameters(texts: list[AscText]) -> dict[str, float]:
    params = {}
    param_patterns = {
    ...
    unit_multipliers = {
        ...
        'k': 1e3, 'meg': 1e6, 'g': 1e9, 't': 1e12, 'mil': 25.4e-6 # Added more multipliers
    }
    ...
    # Calculate Rrtd if all necessary parameters are found
    if all(k in params for k in ["T1", "A", "B", "C", "R0"]):
        T1 = params["T1"]
        A = params["A"]
        B = params["B"]
        C = params["C"]
...
def parse_asc(text: str) -> AscSchematic:
    ...
    for line in lines:
        ...
        # Reset current context if a new top-level section (like SHEET, WIRE etc.) starts
        # This prevents SYMATTR from being incorrectly associated with previous components.
        if kind in {"Version", "SHEET", "WIRE", "FLAG", "TEXT"}:
            current = None
            if kind == "Version" and len(tokens) >= 2:
                version = tokens[1]
            elif kind == "SHEET" and len(tokens) >= 4:
                sheet = (int(tokens[1]), int(tokens[2]), int(tokens[3]))
            elif kind == "WIRE" and len(tokens) >= 5:
                wires.append(
                    AscWire(
            ...
            continue # Continue to next line after processing a top-level section
        ...
        if kind == "SYMBOL" and len(tokens) >= 5:
            ...
            current = comp # New component context
            continue
        ...
        # If it's a line that doesn't start a new top-level section or a new SYMBOL,
        # and it's not a SYMATTR for the current component, then it's either:
        # 1. A WINDOW line (which belongs to the current component, but doesn't define its name/symbol)
        # 2. An unrecognized line that we should simply skip/ignore for parsing purposes.
        # In either case, 'current' should NOT be reset here. We only reset 'current' 
        # when a new SYMBOL is found, or a new top-level section starts (handled above).
        # Therefore, no 'current = None' here.

    params = _parse_parameters(texts)
...
def schematic_to_circuit(schematic: AscSchematic) -> Circuit:
    ...
    # The lm741.sub include needs to be added for OpAmpSubckt
    # Using importlib.resources.files for robust path resolution
    from importlib.resources import files
    try:
        lm741_sub_path = files("spicelab.library.data.opamps").joinpath("lm741.sub")
        if lm741_sub_path.exists():
            circuit.add_directive(f".include {lm741_sub_path.resolve()}")
        else:
            # Fallback if the path doesn't exist, assume LTspice will find it
            circuit.add_directive(".include lm741.sub")
    except Exception:
        # Fallback if importlib.resources fails, assume LTspice will find it
        circuit.add_directive(".include lm741.sub")


    if all("SpiceLine" in comp.attributes for comp in schematic.components):
        # This branch should now be able to handle includes
        ...
        # Fallback to geometry-based parsing, but with improved opamp handling
        uf = _UnionFind()
        ...
        print(f"uf.groups() before _net_for processing: {uf.groups()}") # Debug print

        label_map: dict[tuple[int, int], str] = {}
        for label in schematic.labels:
            rep = uf.find(label.position)
            label_map[rep] = label.name
        print(f"DEBUG: label_map after all labels processed: {label_map}") # Debug print

        print(f"DEBUG: uf.groups() after all adds and unions: {uf.groups()}") # Debug print
        ...
        for comp in schematic.components:
            ...
            # Resolve parameters in component attributes
            # Only resolve "Value" attribute for now
            if "Value" in comp.attributes and isinstance(comp.attributes["Value"], str):
                for param_name, param_value in schematic.params.items():
                    if f"{{{param_name}}}" in comp.attributes["Value"]:
                        comp.attributes["Value"] = comp.attributes["Value"].replace(f"{{{param_name}}}", str(param_value))

            cat_component = spec.create_component(comp)
            circuit.add(cat_component)

            # Ensure pins are correctly set before connecting
            if not comp.pins:
                comp.pins = _absolute_pins(comp, spec)
            
            # Debug prints for pins and their representatives
            print(f"DEBUG: Component: {comp.ref or comp.symbol}")
            for i, pin in enumerate(comp.pins):
                rep = uf.find(pin)
                assigned_net_name = _net_for(pin).name # Call _net_for to see what name it gets
                print(f"  DEBUG: Pin {i} at {pin.x},{pin.y} -> UF_rep: {rep} -> Assigned Net Name: {assigned_net_name}")
